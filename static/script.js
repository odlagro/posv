...
function adicionarAoCarrinho(prod) {
  ...
  if (existente) {
    ...
    if (!existente.imagem_url && prod.imagem_url) existente.imagem_url = prod.imagem_url;
  } else {
    carrinho.push({
      id: prod.id,
      nome: prod.nome,
...
function registrarEventos() {
  ...
  // ORÇAMENTO (ETAPA 2) - gera imagem PNG local e abre/mostra
  const btnOrc = document.getElementById('btn-gerar-orcamento');
  if (btnOrc) {
    btnOrc.addEventListener('click', async () => {
      try {
        btnOrc.disabled = true;
        btnOrc.textContent = 'Gerando...';
        ...
        const resp = await fetch('/api/orcamento/gerar', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
          itens,
          frete: (window.freteSelecionado && window.freteSelecionado.preco) ? Number(window.freteSelecionado.preco) : 0,
          frete_nome: (window.freteSelecionado && window.freteSelecionado.nome) ? String(window.freteSelecionado.nome) : '',
        }),
        });

        const data = await resp.json().catch(() => ({}));
        if (!resp.ok || !data.ok) {
          alert((data && (data.error || data.detail)) || 'Falha ao gerar orçamento.');
          return;
        }

        const url = data.url;
        const img = document.getElementById('orcamento-preview');
        const wrap = document.getElementById('orcamento-preview-wrap');
        if (img) img.src = url;
        if (wrap) wrap.style.display = 'block';

        window.open(url, '_blank');
      } catch (e) {
        alert('Erro ao gerar orçamento: ' + (e && e.message ? e.message : e));
      } finally {
        btnOrc.disabled = false;
        btnOrc.textContent = 'Gerar imagem do orçamento';
      }
    });
  }
}
